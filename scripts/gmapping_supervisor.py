...
def talker():
    ...
    node = roslaunch.core.Node(package,
                               executable,
                               name='gmapping_node',
                               args='_base_frame:=ROSbot2_base _odom_frame:=odom _delta:=0.05 _xmin:=-25 _ymin:=-25 _xmax:=25 _ymax:=25 _maxUrange:=5 _map_update_interval:=1 _linearUpdate:=0.2 _angularUpdate:=0.1',
                               output='screen')
